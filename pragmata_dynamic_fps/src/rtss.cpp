...
bool set_fps_limit(uint32_t fps, char* message, size_t message_size) {
    if (!initialize()) {
        snprintf(message, message_size,
                 "[BL4 Dynamic FPS] RTSS is unavailable or incompatible");
        return false;
    }
    if (!initialize_profile()) {
        snprintf(message, message_size,
                 "[BL4 Dynamic FPS] Failed to determine the game profile");
        return false;
    }
    ...
    if (!g_state.set_property("FramerateLimitDenominator",
        ...
        !g_state.set_property("FramerateLimit", reinterpret_cast<LPBYTE>(&value),
                              sizeof(value))) {
        snprintf(message, message_size,
                 "[BL4 Dynamic FPS] Failed to update RTSS profile %s",
                 g_state.profile);
        return false;
    }
    ...
    snprintf(message, message_size,
             "[BL4 Dynamic FPS] RTSS profile %s set to %u FPS",
